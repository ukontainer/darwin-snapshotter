...
func NewSnapshotter(root string) (snapshots.Snapshotter, error) {
	...
	if err := os.Mkdir(filepath.Join(root, imageDirName), 0700); err != nil && !os.IsExist(err) {
		return nil, fmt.Errorf("failed to create image directory: %s, %w", imageDirName, err)
	}
...
func (o *snapshotter) Remove(ctx context.Context, key string) (err error) {
	...
	if err := os.RemoveAll(image); err != nil {
		return fmt.Errorf("failed to delete image '%s': %w", image, err)
	}
...
func (o *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ []mount.Mount, err error) {
	...
	if parent == "" {
		if err := hdiu.CreateSparseBundle(o.getImagePath(s.ID), imageSizeMB); err != nil {
			return nil, fmt.Errorf("CreateSparseBundle failed: %w", err)
		}
	} else {
		...
		if err := fs.CopyDir(o.getImagePath(s.ID), o.getImagePath(s.ParentIDs[0]), copyDirOpts...); err != nil {
			return nil, fmt.Errorf("failed copy parent layer: %w", err)
		}
	}
